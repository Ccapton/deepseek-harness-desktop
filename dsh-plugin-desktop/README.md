...
Use a native Linux machine with Git and Node `22.19+` or `24+`. From a fresh checkout, run:
...
`dist:linux` refuses non-Linux hosts and non-`x64`/`arm64` Node, runs a Linux gate containing the build, all TypeScript compiler faces, and the Linux packaging and desktop-integration tests, then builds an unsigned AppImage. Version `2.0.9` is written to `dsh-plugin-desktop/dist/DSH Desktop-2.0.9.AppImage`.
