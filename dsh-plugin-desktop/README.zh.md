...
请使用原生 Linux 电脑，并安装 Git 与 Node `22.19+` 或 `24+`。在最新的 checkout 中执行：
...
`dist:linux` 会拒绝非 Linux 宿主以及非 `x64`/`arm64` 的 Node，先执行包含 build、全部 TypeScript compiler face，以及 Linux 打包与桌面集成聚焦测试的 gate，然后构建未签名的 AppImage。版本 `2.0.9` 会输出到 `dsh-plugin-desktop/dist/DSH Desktop-2.0.9.AppImage`。
